...
//			System.out.println(p1+" "+p2+" "+cost);			
	...
	public LinkedList<QueueItem> performUCS(Graph graph){
		...
		for(int i=1;;i++){
			...
			LinkedList<QueueItem> longList = new LinkedList<QueueItem>();
			longList = getListOfLongPath(items, goalState);
			if(longList!=null){
				for(QueueItem queueItem: longList){
					listofPathsToGoal.add(index, queueItem);
					index++;
				}
			}
			listofPathsToGoal = queue.sortPriorityQueue(listofPathsToGoal);
			if(i > 1200){
				if((listofPathsToGoal.size()) > 0 && isTheGoalState(listofPathsToGoal, goalState)){
					Graph.printOptimalPath(listofPathsToGoal, path[0], goalState, i);
					break;	
				}else{
					System.out.println("path from '"+path[0]+"_to_"+goalState+"' is not found");
					break;	
				}
			}	
		}
	 ...
	 * 
	 */
	public static LinkedList<QueueItem> getListOfLongPath(LinkedList<QueueItem> items, String goalState){
		LinkedList<QueueItem> listOFlongestPaths = new LinkedList<QueueItem>();
		for(QueueItem item : items){
			if(getCurrentNode(item).equalsIgnoreCase(goalState)){
				listOFlongestPaths.add(item);
			}
		}
